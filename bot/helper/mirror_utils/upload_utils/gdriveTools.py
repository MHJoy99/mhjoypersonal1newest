...
class GoogleDriveHelper:
    ...
    def __upload_empty_file(self, path, file_name, mime_type, parent_id=None):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Uploaded using MHJoyBots Mirror Bot',
            'mimeType': mime_type,
        }
    ...
    def upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Uploaded by MHJoyBots Mirror Bot',
            'mimeType': mime_type,
        }
        try:
    ...
    def edit_telegraph(self):
        ...
        for content in self.telegraph_content :
            ...
            Telegraph(access_token=telegraph_token).edit_page(path = self.path[prev_page],
                                 title = 'MHJoyBots Mirror Bot Search',
                                 author_name='mhjoy99',
        ...
        if response["files"]:
            ...
            for content in self.telegraph_content :
                self.path.append(Telegraph(access_token=telegraph_token).create_page(
                                                        title = 'MHJoyBots Mirror Bot Search',
                                                        author_name='mhjoy99',
                                                        author_url='https://github.com/breakdowns/mhjoy99',
                                                        html_content=content
                                                        )['path'])
